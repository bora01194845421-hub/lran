...
PROMPT = """5월 29일(목)~6월 1일(일·오늘)간 수집된 데이터를 종합해서 수원시 민생경제 분석과 우선 대응과제를 JSON으로 작성하세요.

★ 이번 주(6/1) 반드시 반영해야 할 핵심 사실 (5/29~6/1 확인된 사실):
① 트럼프, 5/31 이란 MOU 서명 거부 → 핵·호르무즈 조항 강화한 수정안 재발송 (CNN·뉴스핌)
② 5/29 백악관 상황실 회의 2시간 → 결론 없이 종료 (NYT·Times of Israel)
③ 5/29 IRGC 해군, 호르무즈 인근 선박 4척에 경고 사격 — 협상 중에도 통제권 과시
④ 미 국방장관 5/31: "협상 결렬 시 대이란 군사 재개 준비 완료" (서울경제)
⑤ CNBC 5/30: "합의돼도 호르무즈 원유 수출량 전쟁 전 수준 회복 불가" — 우회로 정착
⑥ 유가: 브렌트유 $90대 초반 (5/18 협상 낙관론 이후 -10% 하락했다가 수정안 재발송으로 반등 압력)
⑦ 미 재무부, 이란 암호화폐 $10억 압류 발표 (베센트 장관)
⑧ 전쟁권한법(War Powers Act) 90일 의회 승인 마감 5/29~31 도래 — 트럼프 "휴전으로 시계 멈춤" 주장, 야당 "무효" 반박
⑨ 이란 관리: "트럼프 기준도 없고 변덕스러워, 최종 결정 불가" (NBC)

위 ①~⑨를 분석 근거로 명시적으로 활용할 것. 지난주(5/26) 분석과 중복되는 내용은 작성 금지.

[국제 전황 요약 — 5/29~6/1 수집 핵심 기사]
{war_summary}
...
⚠️ 작성 지침:
1. 지난주와 동일한 제목·내용 반복 금지 — 위 ①~⑨ 사실에 근거한 새로운 분석만 작성
2. "트럼프 수정안 재발송"이라는 새로운 변수가 수원시 에너지·산업·민생에 미치는 구체적 영향 분석
3. "호르무즈 수출 미회복 가능성(CNBC)"이 수원시 중장기 에너지비용 구조에 미치는 영향 반드시 포함
4. 전문가_의견은 유튜브 브리핑 실제 내용을 구체적으로 인용할 것
5. 수치는 위 ①~⑨에 명시된 최신값 사용 (5/31 기준 브렌트 $90대, 환율 등)
6. 지난주 우선과제와 완전히 다른 제목·내용 사용
7. 지난주 "다음주 주목이슈"로 예고된 사건들(트럼프 최종 결정, 의회 마감 등)의 실제 결과를 이번주 내용에 반영
