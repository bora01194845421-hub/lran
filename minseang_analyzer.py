...
PROMPT = """지난 주말(토·일)과 오늘(월)을 포함한 최근 3~5일간 수집된 데이터를 종합해서 수원시 민생경제 분석과 우선 대응과제를 JSON으로 작성하세요.
이번 실행은 주말 동안 새로 발생한 이슈(협상 결렬·전황 변화·국제 기관 발표·유가 급등락)를 반드시 반영해야 합니다.

[국제 전황 요약 — 주말~오늘 수집 핵심 기사]
{war_summary}
...
⚠️ 작성 지침:
1. 지난주와 동일한 제목·내용 반복 금지 — 반드시 주말~오늘 새로운 데이터·사건·수치를 근거로 작성
2. 주말 핵심 신규 사항(이란-미국 협상 동향, 호르무즈 봉쇄 위협, IEA 비축유 방출, 유가 급등락, 새로운 제재·외교 발표 등)을 반드시 반영
3. 전문가_의견은 유튜브 브리핑 실제 내용을 구체적으로 인용할 것 (없으면 최신 전문가 코멘트 활용)
4. 수치·지표는 주말~오늘 기준 최신값 사용
5. 지난주 우선과제 제목이 이번주에 그대로 반복되면 안 됨 — 제목부터 완전히 새로운 표현 사용
6. 지난주 "다음주 주목이슈"에 예고된 사건들의 실제 진행 상황과 수원시 영향을 이번주 내용에 구체적으로 반영
7. 주말 이후 변화된 글로벌 전황(협상·군사 동향·에너지 시장 변동)이 수원시에 미치는 구체적 파급효과를 명시할 것
