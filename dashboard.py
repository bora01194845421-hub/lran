...
<div class="intel-header">
  <div class="intel-header-inner">
    ...
    <div class="header-right">
      {'<a href="?view=desktop" class="mobile-toggle-btn active">🖥 PC 보기</a>' if _mobile_view else '<a href="?view=mobile" class="mobile-toggle-btn">📱 모바일 보기</a>'}
    </div>
  </div>
</div>
...
# ⑤ 한국 지자체 대응 현황
# ═══════════════════════════════════════════════════════════
# 수집된 실제 지자체 대응 데이터만 사용 (DEFAULT 하드코딩 제거)
lga_data_raw = domestic.get("lga_responses", []) or minseang.get("lga_responses", [])

# confirmed_action(신형) 또는 actions(구형) 필드 통합 처리
def _lga_action(row: dict) -> str:
    return row.get("confirmed_action") or row.get("actions") or ""

# 실제 대응 내용이 있는 항목만 필터
lga_list = [
    r for r in lga_data_raw
    if _lga_action(r).strip()
    and not any(m in _lga_action(r) for m in ["수집된 사례 없음", "확인되지 않음", "수집 기사 없음"])
]
...
if lga_list:
    ...
    for row in lga_list:
        ...
        acts  = _lga_action(row)
        evidence = row.get("evidence","")
        acts_html = f"{acts}<br><span style='font-size:0.7rem;color:#6B7280'>{evidence}</span>" if evidence else acts
        lga_rows_html += (
            f'<tr class="lga-row">'
            ...
            f'<td class="lga-action-cell">{acts_html}</td>'
            f'</tr>'
        )
...
<div class="section-card">
  <div class="sec-header">
    ...
    <span class="sec-badge badge-green">수집 기사 기반 확인 사례만 표시</span>
  </div>
  <table class="lga-table">
    <thead>
      <tr>
        ...
        <th class="lga-th">확인된 조치 (출처 포함)</th>
      </tr>
    </thead>
    <tbody>{lga_rows_html}</tbody>
  </table>
  <div style="margin-top:10px;font-size:0.65rem;color:#2D4A65">
    ※ 수집된 기사에서 확인된 사례만 표시. 데이터 미수집 지자체는 생략.
  </div>
</div>
""", unsafe_allow_html=True)
# 수집된 사례 없으면 섹션 전체 숨김 (미표시)
